...
// 	assign finsh_wr = end_wr_busy_posedge_cnt;
	...
	reg [STATE_W-1:0] state_n;//next_state
	reg [STATE_W-1:0] state_c;//current_state
	...
	wire IDLE2ARBITE_start;      
	wire ARBITE2READ_start;     
	wire ARBITE2WRITE_start;    
	wire ARBITE2IDLE_start;      
	wire READ2WAIT_READ_start;   
	wire WRITE2WAIT_WRITE_start; 
	wire WAIT_READ2ARBITE_start; 
	wire WAIT_WRITE2ARBITE_start;

	//设计转移条件
	assign IDLE2ARBITE_start       = (state_c == IDLE       && start_one_new_txn == 1'b1);
	assign ARBITE2READ_start       = (state_c == ARBITE     && enable_read  == 1'b1);
	assign ARBITE2WRITE_start      = (state_c == ARBITE     && enable_write == 1'b1);
	assign ARBITE2IDLE_start       = (state_c == ARBITE     && end_flag     == 1'b1);
	assign READ2WAIT_READ_start    = (state_c == READ                              );
	assign WAIT_READ2ARBITE_start  = (state_c == WAIT_READ  && rd_done_flag == 1'b1);
	assign WRITE2WAIT_WRITE_start  = (state_c == WRITE                             );
	assign WAIT_WRITE2ARBITE_start = (state_c == WAIT_WRITE && wr_done_flag == 1'b1);
	    ...
	    if((~arstn) || (start_one_new_txn == 1'b1)) begin
	    	...
	    	if (clear_flag == 1'b1) begin //清零缓存
	    		if(finsh_wr_r && wr_fifo_empty) begin
	    			...
	    			if((wr_fifo_rd_data_count >= 128) || (finsh_wr_r && (~wr_fifo_empty))) begin
	    				end_flag     <= 1'b0;
	    				enable_write <= 1'b1;
	    				enable_read  <= 1'b0;
	    			end
	    		...
	    		if(finsh_wr_r && wr_fifo_empty && finsh_rd_r) begin //读完成与写完成，确保所有数据已经读取到和写到DDR
	    			...
	    			if (wr_fifo_rd_data_count >= 130 || (finsh_wr_r && (~wr_fifo_empty))) begin
	    				end_flag     <= 1'b0;
	    				enable_write <= 1'b1;
	    				enable_read  <= 1'b0;
	    			end 
	    			else if(rd_fifo_wr_data_count < 126 && (~finsh_rd_r)) begin
	    				end_flag     <= 1'b0;
	    				enable_write <= 1'b0;
	    				enable_read  <= 1'b1;
	    			end
	...
	always@(*)begin
	    case(state_c)
	        ...
	        ARBITE:begin
	            if (ARBITE2READ_start) begin
	                state_n = READ;
	            end
	            else if (ARBITE2WRITE_start) begin
	            	state_n = WRITE;
	            end
	            else if (ARBITE2IDLE_start) begin
	            	state_n = IDLE;
	            end
	            else begin
	                state_n = state_c;
	        ...
	        WAIT_READ:begin
	        	if(WAIT_READ2ARBITE_start)begin
	                state_n = ARBITE;
	            end
	            else begin
	                state_n = state_c;
	        ...
	        WRITE:begin
	        	if(WRITE2WAIT_WRITE_start)begin
	                state_n = WAIT_WRITE;
	            end
	            else begin
	                state_n = state_c;
	        ...
	        WAIT_WRITE:begin
	        	if(WAIT_WRITE2ARBITE_start)begin
	                state_n = ARBITE;
	            end
	            else begin
	                state_n = state_c;
