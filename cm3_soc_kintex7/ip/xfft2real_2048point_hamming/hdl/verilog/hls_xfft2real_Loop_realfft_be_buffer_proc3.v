// ==============================================================
// RTL generated by Vitis HLS - High-Level Synthesis from C, C++ and OpenCL v2020.2 (64-bit)
// Version: 2020.2
// Copyright (C) Copyright 1986-2020 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

`timescale 1 ns / 1 ps 

module hls_xfft2real_Loop_realfft_be_buffer_proc3 (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_continue,
        ap_idle,
        ap_ready,
        descramble_buf_0_M_imag_V_address0,
        descramble_buf_0_M_imag_V_ce0,
        descramble_buf_0_M_imag_V_we0,
        descramble_buf_0_M_imag_V_d0,
        descramble_buf_1_M_imag_V_address0,
        descramble_buf_1_M_imag_V_ce0,
        descramble_buf_1_M_imag_V_we0,
        descramble_buf_1_M_imag_V_d0,
        din_V_TDATA,
        din_V_TVALID,
        din_V_TREADY,
        descramble_buf_0_M_real_V_address0,
        descramble_buf_0_M_real_V_ce0,
        descramble_buf_0_M_real_V_we0,
        descramble_buf_0_M_real_V_d0,
        descramble_buf_1_M_real_V_address0,
        descramble_buf_1_M_real_V_ce0,
        descramble_buf_1_M_real_V_we0,
        descramble_buf_1_M_real_V_d0
);

parameter    ap_ST_fsm_state1 = 2'd1;
parameter    ap_ST_fsm_state2 = 2'd2;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
input   ap_continue;
output   ap_idle;
output   ap_ready;
output  [8:0] descramble_buf_0_M_imag_V_address0;
output   descramble_buf_0_M_imag_V_ce0;
output   descramble_buf_0_M_imag_V_we0;
output  [31:0] descramble_buf_0_M_imag_V_d0;
output  [8:0] descramble_buf_1_M_imag_V_address0;
output   descramble_buf_1_M_imag_V_ce0;
output   descramble_buf_1_M_imag_V_we0;
output  [31:0] descramble_buf_1_M_imag_V_d0;
input  [63:0] din_V_TDATA;
input   din_V_TVALID;
output   din_V_TREADY;
output  [8:0] descramble_buf_0_M_real_V_address0;
output   descramble_buf_0_M_real_V_ce0;
output   descramble_buf_0_M_real_V_we0;
output  [31:0] descramble_buf_0_M_real_V_d0;
output  [8:0] descramble_buf_1_M_real_V_address0;
output   descramble_buf_1_M_real_V_ce0;
output   descramble_buf_1_M_real_V_we0;
output  [31:0] descramble_buf_1_M_real_V_d0;

reg ap_done;
reg ap_idle;
reg ap_ready;
reg descramble_buf_0_M_imag_V_ce0;
reg descramble_buf_0_M_imag_V_we0;
reg descramble_buf_1_M_imag_V_ce0;
reg descramble_buf_1_M_imag_V_we0;
reg descramble_buf_0_M_real_V_ce0;
reg descramble_buf_0_M_real_V_we0;
reg descramble_buf_1_M_real_V_ce0;
reg descramble_buf_1_M_real_V_we0;

reg    ap_done_reg;
(* fsm_encoding = "none" *) reg   [1:0] ap_CS_fsm;
wire    ap_CS_fsm_state1;
reg    din_V_TDATA_blk_n;
wire    ap_CS_fsm_state2;
wire   [9:0] i_fu_128_p2;
reg    ap_block_state2;
reg   [9:0] i_121_reg_114;
reg    ap_block_state1;
wire   [0:0] icmp_ln83_fu_178_p2;
wire   [63:0] zext_ln89_fu_162_p1;
wire   [0:0] tmp_fu_170_p3;
wire   [31:0] trunc_ln145_fu_134_p1;
reg   [8:0] trunc_ln5_fu_152_p4;
reg   [1:0] ap_NS_fsm;
wire    regslice_both_din_V_U_apdone_blk;
wire   [63:0] din_V_TDATA_int_regslice;
wire    din_V_TVALID_int_regslice;
reg    din_V_TREADY_int_regslice;
wire    regslice_both_din_V_U_ack_in;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_done_reg = 1'b0;
#0 ap_CS_fsm = 2'd1;
end

hls_xfft2real_regslice_both #(
    .DataWidth( 64 ))
regslice_both_din_V_U(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .data_in(din_V_TDATA),
    .vld_in(din_V_TVALID),
    .ack_in(regslice_both_din_V_U_ack_in),
    .data_out(din_V_TDATA_int_regslice),
    .vld_out(din_V_TVALID_int_regslice),
    .ack_out(din_V_TREADY_int_regslice),
    .apdone_blk(regslice_both_din_V_U_apdone_blk)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_state1;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_done_reg <= 1'b0;
    end else begin
        if ((ap_continue == 1'b1)) begin
            ap_done_reg <= 1'b0;
        end else if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (icmp_ln83_fu_178_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state2))) begin
            ap_done_reg <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (icmp_ln83_fu_178_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state2))) begin
        i_121_reg_114 <= i_fu_128_p2;
    end else if (((~((ap_start == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state1)) | (~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (icmp_ln83_fu_178_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state2)))) begin
        i_121_reg_114 <= 10'd0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (icmp_ln83_fu_178_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state2))) begin
        ap_done = 1'b1;
    end else begin
        ap_done = ap_done_reg;
    end
end

always @ (*) begin
    if (((ap_start == 1'b0) & (1'b1 == ap_CS_fsm_state1))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (icmp_ln83_fu_178_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state2))) begin
        ap_ready = 1'b1;
    end else begin
        ap_ready = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_0_M_imag_V_ce0 = 1'b1;
    end else begin
        descramble_buf_0_M_imag_V_ce0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (tmp_fu_170_p3 == 1'd0) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_0_M_imag_V_we0 = 1'b1;
    end else begin
        descramble_buf_0_M_imag_V_we0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_0_M_real_V_ce0 = 1'b1;
    end else begin
        descramble_buf_0_M_real_V_ce0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (tmp_fu_170_p3 == 1'd0) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_0_M_real_V_we0 = 1'b1;
    end else begin
        descramble_buf_0_M_real_V_we0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_1_M_imag_V_ce0 = 1'b1;
    end else begin
        descramble_buf_1_M_imag_V_ce0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (tmp_fu_170_p3 == 1'd1) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_1_M_imag_V_we0 = 1'b1;
    end else begin
        descramble_buf_1_M_imag_V_we0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_1_M_real_V_ce0 = 1'b1;
    end else begin
        descramble_buf_1_M_real_V_ce0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (tmp_fu_170_p3 == 1'd1) & (1'b1 == ap_CS_fsm_state2))) begin
        descramble_buf_1_M_real_V_we0 = 1'b1;
    end else begin
        descramble_buf_1_M_real_V_we0 = 1'b0;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state2))) begin
        din_V_TDATA_blk_n = din_V_TVALID_int_regslice;
    end else begin
        din_V_TDATA_blk_n = 1'b1;
    end
end

always @ (*) begin
    if ((~((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state2))) begin
        din_V_TREADY_int_regslice = 1'b1;
    end else begin
        din_V_TREADY_int_regslice = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_state1 : begin
            if ((~((ap_start == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state1))) begin
                ap_NS_fsm = ap_ST_fsm_state2;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state1;
            end
        end
        ap_ST_fsm_state2 : begin
            ap_NS_fsm = ap_ST_fsm_state2;
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign ap_CS_fsm_state1 = ap_CS_fsm[32'd0];

assign ap_CS_fsm_state2 = ap_CS_fsm[32'd1];

always @ (*) begin
    ap_block_state1 = ((ap_start == 1'b0) | (ap_done_reg == 1'b1));
end

always @ (*) begin
    ap_block_state2 = ((ap_start == 1'b0) | (din_V_TVALID_int_regslice == 1'b0) | (ap_done_reg == 1'b1));
end

assign descramble_buf_0_M_imag_V_address0 = zext_ln89_fu_162_p1;

assign descramble_buf_0_M_imag_V_d0 = {{din_V_TDATA_int_regslice[63:32]}};

assign descramble_buf_0_M_real_V_address0 = zext_ln89_fu_162_p1;

assign descramble_buf_0_M_real_V_d0 = trunc_ln145_fu_134_p1;

assign descramble_buf_1_M_imag_V_address0 = zext_ln89_fu_162_p1;

assign descramble_buf_1_M_imag_V_d0 = {{din_V_TDATA_int_regslice[63:32]}};

assign descramble_buf_1_M_real_V_address0 = zext_ln89_fu_162_p1;

assign descramble_buf_1_M_real_V_d0 = trunc_ln145_fu_134_p1;

assign din_V_TREADY = regslice_both_din_V_U_ack_in;

assign i_fu_128_p2 = (i_121_reg_114 + 10'd1);

assign icmp_ln83_fu_178_p2 = ((i_121_reg_114 == 10'd1023) ? 1'b1 : 1'b0);

assign tmp_fu_170_p3 = i_121_reg_114[32'd0];

assign trunc_ln145_fu_134_p1 = din_V_TDATA_int_regslice[31:0];

integer ap_tvar_int_0;

always @ (i_121_reg_114) begin
    for (ap_tvar_int_0 = 9 - 1; ap_tvar_int_0 >= 0; ap_tvar_int_0 = ap_tvar_int_0 - 1) begin
        if (ap_tvar_int_0 > 9 - 1) begin
            trunc_ln5_fu_152_p4[ap_tvar_int_0] = 1'b0;
        end else begin
            trunc_ln5_fu_152_p4[ap_tvar_int_0] = i_121_reg_114[9 - ap_tvar_int_0];
        end
    end
end

assign zext_ln89_fu_162_p1 = trunc_ln5_fu_152_p4;

endmodule //hls_xfft2real_Loop_realfft_be_buffer_proc3
